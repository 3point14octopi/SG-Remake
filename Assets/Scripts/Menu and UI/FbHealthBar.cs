...
public class FbHealthBar : MonoBehaviour
{
    ...
    public bool splitHead; //acts as a bool to remember if 
    ...
    public void HealthBar(float health)
    {
        ...
        else //for if max health is even
        {
            //sets our full heads (number of times 2 goes into our health)
            for (int i = head; i < (Mathf.FloorToInt(health) / 2); i++) { heads[i].sprite = fullHead; head++; }
    ...
    public void IncreaseHealthBar(float max, float currentHealth)
    {
        ...
        else if (Mathf.FloorToInt(max) % 2 == 0) splitHead = false;
        maxHealth = max;
        HealthBar(currentHealth);
    }

    // When our max health is decreased, this will remove hearts from the UI
    public void DecreaseHealthBar(float max, float currentHealth)
    {
        while (heads.Count > max / 2f) // removes heads until we have 1 for every 2 health
        {
            // removes the last UI icon and removes it from our array
            Image lastHead = heads[heads.Count - 1];
            heads.RemoveAt(heads.Count - 1);
            Destroy(lastHead.gameObject);
        }

        // stores if the max health is even (all full heads) or odd (has a half icon)
        if (Mathf.FloorToInt(max) % 2 == 1) splitHead = true;
        else if (Mathf.FloorToInt(max) % 2 == 0) splitHead = false;
        maxHealth = max;
        HealthBar(currentHealth);
